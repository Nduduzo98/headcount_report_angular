...
@Component({
  selector: 'app-tn-inprogress',
  templateUrl: './tn-inprogress.component.html',
  styleUrls: ['./tn-inprogress.component.scss']
})
export class TnInprogressComponent {
  ...
  ngOnInit(): void {
    this.report.getInProgressByDistrict(this.districtID).subscribe({
      next:(data)=>{
  ...
  download(){
    this.report.downloadInProgress(this.districtID).subscribe({
  ...
  downloadExcel(){
    this.report.downloadInprogressExcel(this.districtID).subscribe(
      (excelBlob: Blob) => {
